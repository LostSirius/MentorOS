...
export const uploadFile = async (
  ...
  // Prefer ASCII-safe path segments (UUID ids). Fall back to unicode-safe btoa.
  const safeId = /^[a-zA-Z0-9._-]+$/.test(payload.file_id)
    ? payload.file_id
    : btoa(unescape(encodeURIComponent(payload.file_id)))
      .replace(/\+/g, "-")
      .replace(/\//g, "_")
      .replace(/=+$/, "")

  const filePath = `${payload.user_id}/${safeId}`
