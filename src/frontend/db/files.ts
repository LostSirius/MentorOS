...
export const getFileWorkspacesByWorkspaceId = async (workspaceId: string) => {
  ...
  return workspace as typeof workspace & { files: Tables<"files">[] }
}
...
export const getFileWorkspacesByFileId = async (fileId: string) => {
  ...
  return file as typeof file & { workspaces: Tables<"workspaces">[] }
}
...
export const createFile = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(`Embedding skipped for file ${createdFile.id}: ${errorMessage}`)
      toast.warning(
        `File uploaded, but retrieval indexing failed: ${errorMessage}`
      )
    }
  } catch (e: any) {
    console.warn("Embedding step failed (non-critical), file still usable:", e)
    toast.warning(
      `File uploaded, but retrieval indexing failed: ${e?.message || String(e)}`
    )
  }
...
export const createDocXFile = async (
  ...
  // Match createFile sanitization — Chinese / unicode names break btoa in storage paths.
  let validFilename = fileRecord.name.replace(/[^a-z0-9.]/gi, "_").toLowerCase()
  ...
  const maxBaseNameLength = 100 - extension.length - 1
  if (baseName.length > maxBaseNameLength) {
    fileRecord.name = baseName.substring(0, maxBaseNameLength) + "." + extension
  } else {
    fileRecord.name = (baseName || "document") + "." + extension
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(`Embedding skipped for docx ${createdFile.id}: ${errorMessage}`)
      toast.warning(
        `DOCX uploaded, but retrieval indexing failed: ${errorMessage}`
      )
    }
  } catch (e: any) {
    console.warn("Embedding step failed (non-critical), file still usable:", e)
    toast.warning(
      `DOCX uploaded, but retrieval indexing failed: ${e?.message || String(e)}`
    )
  }
...
export const createFiles = async (
  ...
  await createFileWorkspaces(
    createdFiles.map((file: Tables<"files">) => ({
