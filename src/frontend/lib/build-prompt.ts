...
export async function buildFinalMessages(
  ...
  let systemContent = BUILT_PROMPT
  if (payload.skillSystemPrompt) {
    systemContent = `${payload.skillSystemPrompt}\n\n---\n\n${systemContent}`
  }
...
export async function adaptMessagesForGoogleGemini(
  ...
  // Keep system messages as role "system" so the Google API route can
  // move them into systemInstruction. Do not map system → user.
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    geminiMessages.push(adaptSingleMessageForGoogleGemini(message))
  }
