...
function addApiKeysToProfile(profile: Tables<"profiles">) {
  const apiKeys = {
    ...
    [VALID_ENV_KEYS.SUPERVISOR_SKILLS_API_KEY]: "supervisor_skills_api_key",
  ...
  for (const [envKey, profileKey] of Object.entries(apiKeys)) {
    ...
    if (!envVal) continue
    const current = String((profile as any)[profileKey] || "").trim()
    // Profile override wins; env is only the fallback default.
    if (!current) {
      ;(profile as any)[profileKey] = envVal
    }
  }
