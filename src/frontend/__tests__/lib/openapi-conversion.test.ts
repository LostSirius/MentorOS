...
describe("extractOpenapiData for body 2", () => {
  it("should parse a valid OpenAPI body schema for body 2", async () => {
    ...
    const firstParams =
      functions[0].function.parameters.properties.parameters
    expect(firstParams.properties).toHaveProperty("stocksTicker")
    expect(firstParams.properties.stocksTicker.type).toBe("string")
    expect(firstParams.properties).toHaveProperty("date")
    expect(firstParams.properties.date.type).toBe("string")
    expect(firstParams.properties.date).toHaveProperty("format", "date")
    expect(firstParams.required).toEqual(
      expect.arrayContaining(["stocksTicker", "date"])
    )
    ...
    const secondParams =
      functions[1].function.parameters.properties.parameters
    expect(secondParams.properties).toHaveProperty("stocksTicker")
    expect(secondParams.properties.stocksTicker.type).toBe("string")
    expect(secondParams.required).toContain("stocksTicker")
  })
})
